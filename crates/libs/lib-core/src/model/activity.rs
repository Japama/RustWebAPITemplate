...
mod tests {
    ...
    use crate::ctx::Ctx;
    use crate::model::activity::{ActivityBmc, ActivityForCreate, ActivityForUpdate, Category};
    ...
    async fn test_list_activities_ok() -> Result<()> {
        ...
        ActivityBmc::create(&ctx, &mm, activity_c1).await?;
    ...
    async fn test_update_activity_ok() -> Result<()> {
        ...
        // -- Exec
        ActivityBmc::update(&ctx, &mm, &id, activity_u).await?;
        let activity = ActivityBmc::get(&ctx, &mm, &id).await?;
